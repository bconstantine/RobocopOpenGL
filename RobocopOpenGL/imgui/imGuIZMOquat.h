...
struct imguiGizmo
{
    ...
    quat qtV  = quat(1.0f, vgm::Vec3(0.0f)); // Quaternion value
    quat qtV2 = quat(1.0f, vgm::Vec3(0.0f)); // Quaternion value
...
//      evSuperModifier   -> Super
    ...
    // vgm::Vec3 -> quat -> trackbalTransforms -> quat -> vgm::Vec3
    ////////////////////////////////////////////////////////////////////////////
    bool getTransforms(quat& q, const char* label, vgm::Vec3& dir, float size) {
        ...
        if (ret) dir = (q * vgm::Vec3(1.0f, 0.0f, 0.0f)) * len ; //return vector with original lenght

        return ret;
    }
    // vgm::Vec4 (xyz axis, w angle) -> quat -> trackbalTransforms -> quat -> vgm::Vec4
    ////////////////////////////////////////////////////////////////////////////
    bool getTransforms(quat& q, const char* label, vgm::Vec4& axis_angle, float size) {
...
IMGUI_API bool gizmo3D(const char*, quat&, vgm::Vec4&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::modeDual|imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char*, quat&, vgm::Vec3&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::modeDual|imguiGizmo::cubeAtOrigin);

#ifndef IMGUIZMO_USE_ONLY_ROT

//with Pan & Dolly feature
IMGUI_API bool gizmo3D(const char*, vgm::Vec3&, quat&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::mode3Axes|imguiGizmo::cubeAtOrigin);
...
IMGUI_API bool gizmo3D(const char*, vgm::Vec3&, quat&, quat&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::modeDual|imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char*, vgm::Vec3&, quat&, vgm::Vec4&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::modeDual|imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char*, vgm::Vec3&, quat&, vgm::Vec3&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::modeDual|imguiGizmo::cubeAtOrigin);
